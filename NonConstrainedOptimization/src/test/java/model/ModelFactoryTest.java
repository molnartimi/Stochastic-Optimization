...
public class ModelFactoryTest {
	...
	public void simpleServerTest() {
		...
		List<String> paramNames = model.parameterList.stream().map(param -> param.getName()).collect(Collectors.toList());
		...
		List<Double> defaultValues = model.parameterHandlerList.stream().map(handler -> handler.defaultValue).collect(Collectors.toList());
		List<Double> expectedDefaultValues = Arrays.asList(new Double[] {1.5, 0.25});
		assertThat(defaultValues, Is.is(expectedDefaultValues));
		
		List<String> rewardNames = model.rewardList.stream().map(reward -> reward.getConfigurationName()).collect(Collectors.toList());
